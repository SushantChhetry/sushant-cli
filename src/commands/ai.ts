...
const STRATEGIES: Record<AiCommandName, string> = {
  wwsd: 'Frame the highest-leverage decision, compare options, and pick the fastest learning path.',
  breakdown:
    'Split the problem into goals, constraints, assumptions, and next experiments with clear owners.',
...
export type AiCommandName = 'wwsd' | 'breakdown' | 'redflag' | 'visualize';

export async function runAiCommand(command: AiCommandName, input: string): Promise<void> {
...
function formatTitle(command: AiCommandName): string {
